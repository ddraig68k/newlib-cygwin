...
#define DUART_ADDR	0x003df501
#define READREG(x)	(*((volatile char *) DUART_ADDR + (x * 2) ))
#define WRITEREG(x, y)	(*((char *) DUART_ADDR + (x * 2) ) = y)

/*
 * raw_outbyte -- send a byte to the DUART buffer. This only sends
 *           to channel A.
 */
static void raw_outbyte (char byte)
{
...
//  WRITEREG (DUART_TBA, byte);		/* write the byte */
    ...
    asm("moveb %0,%%d0" :: "g" (byte));
    asm("move.l	#0x0,\%a3");
    asm("jsr 0x3e812a"); //(VEC_OUT EhBasic)
    asm("movem.l (\%sp)+,\%d0-\%d7/\%a0-\%a6");
//    asm("moveb #6, \%d0");
//    asm("moveb %0,%%d1" :: "g" (byte));
//    asm("trap #15");
}
