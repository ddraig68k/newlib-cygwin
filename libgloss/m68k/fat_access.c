...
int fatfs_init(struct fatfs *fs)
{
    ...
    // For FAT12/FAT16 (which this may be), rootdir_first_cluster is actuall rootdir_first_sector
...
int fatfs_sector_reader(struct fatfs *fs, uint32 start_cluster, uint32 offset, uint8 *target)
{
    ...
    if (fs->fat_type != FAT_TYPE_32 && start_cluster == 0)
    {
...
{
    // FAT12/FAT16 Root directory
    if (fs->fat_type != FAT_TYPE_32 && cluster == 0)
    {
        uint32 lba;

        // In FAT12/FAT16, there are a limited amount of sectors in root dir!
...
{
    // NOTE: On FAT12/FAT16 this will be 0 which has a special meaning...
