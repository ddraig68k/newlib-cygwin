...
extern int media_read_fdc(unsigned long sector, unsigned char *buffer, unsigned long sector_count);
extern int media_read(unsigned long sector, unsigned char *buffer, unsigned long sector_count);
extern int media_write_fdc(unsigned long sector, unsigned char *buffer, unsigned long sector_count);
...
char *ptr=path2;
	media_start();
	if (((strncmp(path,"fd0:",4)) == 0) || ((strncmp(path,"FD0:",4)) == 0) ) {
		if (fl_attach_media(media_read_fdc, media_write_fdc) != FAT_INIT_OK)
		{
			iprintf("Can't open floppy disk. Not FAT12?\n");
			return; 
		}
		path+=4;
	} else if (((strncmp(path,"hd1:",4)) == 0) || ((strncmp(path,"HD1:",4)) == 0) ) {
		slave=1; // Slave
		if (fl_attach_media(media_read, media_write) != FAT_INIT_OK)
		{
			iprintf("Can't open hd1:\n");
			return; 
		}
		path+=4;
	} else {
		if (((strncmp(path,"hd0:",4)) == 0) || ((strncmp(path,"HD0:",4)) == 0) )
		  path+=4;
		slave=0; //Master
		if (fl_attach_media(media_read, media_write) != FAT_INIT_OK)
		{
			iprintf("Can't open hd0:\n");
			return NULL; 
		}
	}
	if (path[0]!='/') {
