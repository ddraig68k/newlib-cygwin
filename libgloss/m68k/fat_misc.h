...
//-----------------------------------------------------------------------------
#define EVEN(n)	((n & 1) != 1)
#define UPPER_12_BITS(n)	(n >> 4) 
#define LOWER_12_BITS(n)	(n & 0x0FFF)
#define FAT12_OFFSET(n)	(n + (n >> 1))	// n + 1/2n = 1,5*n
...
#define GET_LOWER_12BIT_WORD(buffer, location)    ( 0xfff & ((uint16)buffer[location+1]<<8) + (uint16)buffer[location+0] )
#define GET_UPPER_12BIT_WORD(buffer, location)    ( 0xfff & ((uint16)buffer[location+1]<<4) + (0xf & (uint16)buffer[location+0]>>4) )
...
#define SET_LOWER_12BIT_WORD(buffer, location, value)    { buffer[location+0] = (uint8)((value)&0xFF); \
                                                  buffer[location+1] = (uint8)((value>>8)&0x0F) + (buffer[location+1]&0xf0); }
#define SET_UPPER_12BIT_WORD(buffer, location, value)    { buffer[location+0] = (uint8)((value<<4)&0xF0) + (buffer[location+0]&0x0F); \
                                                  buffer[location+1] = (uint8)((value>>4)&0xFF); }
