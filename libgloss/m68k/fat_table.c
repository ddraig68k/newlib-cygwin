...
#define FAT12_GET_LOWER_12BIT_WORD(pbuf, location)        ( GET_LOWER_12BIT_WORD(pbuf->ptr, location) )
#define FAT12_GET_UPPER_12BIT_WORD(pbuf, location)        ( GET_UPPER_12BIT_WORD(pbuf->ptr, location) )
#define FAT12_SET_LOWER_12BIT_WORD(pbuf, location, value) { SET_LOWER_12BIT_WORD(pbuf->ptr, location, value); pbuf->dirty = 1; }
#define FAT12_SET_UPPER_12BIT_WORD(pbuf, location, value) { SET_UPPER_12BIT_WORD(pbuf->ptr, location, value); pbuf->dirty = 1; }
...
uint32 fatfs_find_next_cluster(struct fatfs *fs, uint32 current_cluster)
{
    ...
    if (fs->fat_type == FAT_TYPE_12)
        fat_sector_offset = (3*current_cluster / 1024); // 3 Sektoren à 512Byte (1536Byte). Anzahl der FAT Eintraege = /1.5 
        //fat_sector_offset = 0;
    ...
    if (fs->fat_type == FAT_TYPE_12)
    {
        ...
        position = FAT12_OFFSET(current_cluster - (fat_sector_offset * 170)); // FAT_OFFSET=*1,5

        // Read Next Clusters value from Sector Buffer
	if (EVEN(current_cluster)) {
		// get lower 12 Bit
        	nextcluster = FAT12_GET_LOWER_12BIT_WORD(pbuf, (uint16)position);     
	}
	else
	{
		// get upper 12 Bit
        	nextcluster = FAT12_GET_UPPER_12BIT_WORD(pbuf, (uint16)position);     
	}
...
int fatfs_fat_set_cluster(struct fatfs *fs, uint32 cluster, uint32 next_cluster)
{
    ...
    // Find which sector of FAT table to read
    if (fs->fat_type == FAT_TYPE_12)
        fat_sector_offset = (3*cluster / 1024); // 3 Sektoren à 512Byte (1536Byte). Anzahl der FAT Eintraege = /1.5 
    else if (fs->fat_type == FAT_TYPE_16)
    ...
    {
        // Find 32 bit entry of current sector relating to cluster number 
        position = FAT12_OFFSET(cluster - (fat_sector_offset * 170)); // FAT_OFFSET=*1,5

        // Read Next Clusters value from Sector Buffer
	if (EVEN(cluster)) {
		// set lower 12 Bit
        	//nextcluster = FAT12_GET_LOWER_12BIT_WORD(pbuf, (uint16)position);     
        	FAT12_SET_LOWER_12BIT_WORD(pbuf, (uint16)position, ((uint16)next_cluster));     
	}
	else
	{
		// set upper 12 Bit
        	//nextcluster = FAT12_GET_UPPER_12BIT_WORD(pbuf, (uint16)position);     
        	FAT12_SET_UPPER_12BIT_WORD(pbuf, (uint16)position, ((uint16)next_cluster));     
	}
